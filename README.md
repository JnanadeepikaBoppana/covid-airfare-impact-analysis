...
> > >
> > > - **Q3 2020 was the low point of the period** at $269.21 average fare — a 31.3% drop from the Q4 2019 baseline, the lowest of any quarter analyzed
> > > - - **29.2% fare drop** from Q4 2019 to Q2 2020 — the steepest single-quarter decline, coinciding with peak lockdowns
> > >   - - **Negative correlation of -0.625** between COVID-19 case counts and airfare prices, pooling all 10 states together — as cases rose, fares fell significantly
> > >     - - **Correlation is stronger within individual states** — averaging -0.86 when computed separately for each state and then averaged, peaking at -0.91 in Texas. Pooling all states into one correlation understates the relationship because baseline fare levels differ by state; the state-by-state view is the more representative measure of the case-fare relationship
...
> > > 2. 2. **COVID-19 Cases vs Flight Fares** — Dual-axis time series showing the inverse relationship between cases and fares
> > >    3. 3. **COVID-19 Quarterly Case Trends** — 7-day rolling average of COVID cases across the 5 quarters
> > >       4. 4. **Quarterly Heat Map** — State-by-quarter heat map showing both COVID case intensity and fare price changes
> > >         
> > >          5. > To view locally: Download `dashboard/COVID_Airfare_Dashboard.twbx` and open it in [Tableau Desktop](https://www.tableau.com/products/desktop) or [Tableau Public](https://public.tableau.com/).
> > >             >
> > >             > ---
> > >             >
> > >             > ## Data Sources
...
> > >             > 2. 2. **Cleaned and combined** in Excel — removed empty rows, standardized column names, merged all quarters into a single file
> > >             >    3. 3. **Enhanced with COVID data** — daily state-level COVID-19 case data joined to quarterly airfare data by state
> > >             >       4. 4. **Visualized in Tableau** — 4 interactive dashboards compiled into a Tableau Story
> > >             >         
> > >             >          5. ---
> > >             >         
> > >             >          6. ## Limitations
> > >             >         
> > >             >          7. - Analysis is limited to 10 states due to COVID data availability at the time of the project
...
> > > 2. 2. Or download `dashboard/COVID_Airfare_Dashboard.twbx` and open in Tableau Desktop
> > >    3. 3. Navigate through the dashboard to explore each visualization
> > >      
> > >       4. ---
> > >      
> > >       5. *Project completed as part of the Data Visualization course — MS in Business Analytics*
> > >       6. 
